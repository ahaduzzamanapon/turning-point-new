...
export default function EmployeeAttendanceIndex({ auth, attendances }) {
    const handleDelete = (id) => {
        if (confirm('Are you sure you want to delete this attendance record?')) {
            Inertia.delete(route('admin.employee-attendances.destroy', id));
        }
    };
                                    ...
                                    {attendances.length > 0 ? (
                                        attendances.map((attendance) => (
                                            <tr key={attendance.id}>
                                                <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-white">{attendance.employee.name}</td>
                                                ...
                                                <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                                    <Link href={route('admin.employee-attendances.edit', attendance.id)} className="text-indigo-600 hover:text-indigo-900 mr-3">Edit</Link>
