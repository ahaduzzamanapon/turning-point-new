...
export default function ContinuousAttendance({ selectedEmployees }) {
    ...
    const fetchContinuousAttendance = async () => {
        ...
        try {
            ...
            const response = await axios.get('/api/reports/attendance/continuous', {
                params: {
            ...
            setError('Failed to fetch continuous attendance.');
            console.error(err);
        } finally {
            setLoading(false);
        }
    ...
    const handleExport = async () => {
        try {
            ...
            window.open(`/api/reports/attendance/continuous/export?${params}`, '_blank');
        } catch (err) {
            console.error('Failed to export continuous attendance:', err);
            alert('Failed to export continuous attendance.');
        }
    };
    ...
    return (
        <div className="continuous-attendance-tab">
            <h3 className="text-lg font-semibold mb-4">Continuous Attendance</h3>
            ...
            {loading && <p>Loading continuous attendance data...</p>}
            ...
            {!loading && !error && continuousAttendanceData.length === 0 && (
                <p>No continuous attendance records found for the selected date range and employees.</p>
            )}
        </div>
    );
}