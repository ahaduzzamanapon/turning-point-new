...
export default function LedgerWiseIncomes({ auth, incomes, filters, ledgers }) {
    ...
    const handleFilter = (e) => {
        e.preventDefault();
        get(route('admin.reports.incomes'), data, { preserveState: true });
    };

    return (
        <AuthenticatedLayout
            user={auth.user}
            header={<h2 className="font-semibold text-xl text-white leading-tight">Ledger-wise Incomes Report</h2>}
        >
            <Head title="Ledger-wise Incomes Report" />

            <div className="py-12">
                <div className="max-w-7xl mx-auto sm:px-6 lg:px-8">
                    <div className="bg-black/30 backdrop-blur-xl shadow-lg rounded-lg border border-white/10 p-6 text-white">
                        <h3 className="text-2xl font-bold text-red-700 mb-6">Ledger-wise Incomes Report</h3>
                        ...
                        <form onSubmit={handleFilter} className="mb-6 flex space-x-4">
                            ...
                            <div className="flex items-end">
                                ...
                                <a
                                    href={route('admin.reports.exportLedgerWiseIncomes', data)}
                        ...
                        {Object.keys(incomes).length > 0 ? (
                            Object.keys(incomes).map((ledgerName) => (
                                <div key={ledgerName} className="mb-8">
                                    ...
                                    <p className="text-right text-lg font-bold">Total {ledgerName} Incomes: {incomes[ledgerName].reduce((sum, item) => sum + parseFloat(item.amount), 0).toFixed(2)}</p>
                                </div>
                            ))
                        ) : (
