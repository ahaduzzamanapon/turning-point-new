...
export default function MonthlyStudentAttendance({ selectedStudents }) {
    ...
    }, [selectedMonth, selectedStudents]);

    const fetchMonthlyAttendance = async () => {
        setLoading(true);
        setError(null);
        try {
            const response = await axios.get('/api/reports/student-attendance/monthly', {
                params: {
                    month: selectedMonth,
            ...
            setError('Failed to fetch monthly student attendance.');
            console.error(err);
        } finally {
            setLoading(false);
        }
    ...
    const handleExport = async () => {
        try {
            ...
            window.open(`/api/reports/student-attendance/monthly/export?${params}`, '_blank');
        } catch (err) {
            console.error('Failed to export monthly student attendance:', err);
            alert('Failed to export monthly student attendance.');
        }
    };
    ...
    return (
        <div className="monthly-attendance-tab">
            <h3 className="text-lg font-semibold mb-4">Monthly Student Attendance</h3>
            ...
            {!loading && !error && monthlyAttendanceData.length === 0 && (
                <p>No monthly attendance records found for the selected month and students.</p>
            )}
        </div>
    );
}